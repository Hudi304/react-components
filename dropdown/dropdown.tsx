...
export const DropdownTrigger: FC<DropdownTriggerProps> = ({ children }) => {
  return (
    <Menu.Button as='div' className="border-2 border-black rounded-full w-full px-5 h-10 flex justify-between items-center">
      {children}
      <ChevronDownIcon
...
export const DropdownBody: FC<Children> = ({ children }) => {
  return (
    ...
    >
      <Menu.Items className="dropdown-body">
        {children}
      </Menu.Items>
    </Transition>
  )
...
export const DropdownItem: FC<DropdownItemProps> = ({ children, className = "" }: DropdownItemProps) => {
  return (
    <Menu.Item>
      {({ active, disabled }) => {
        const isActive = active ? 'bg-violet-500 text-white' : 'text-gray-900'
        return (
          <button className={`${isActive} ${className} flex w-full items-center rounded-md px-2 py-2 text-sm`}>
            {children}
          </button>
        )
      }}
